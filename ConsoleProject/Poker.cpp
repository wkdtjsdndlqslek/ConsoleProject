...
{
    int checkPairHighPair[3];//CheckPair로 받은 값을 한 번 받아오기 위한 임시 배열
    CheckPair(n, s, checkPairHighPair);
    double attRatio = 0.0f;//공격력 배율
    int CP = checkPairHighPair[0];
    int HP = checkPairHighPair[1];//HP는 페어 중 높은 숫자

    int triple = checkPairHighPair[2];//triple은 트리플의 숫자 중 높은 숫자로 풀하우스를 위한 것 중 안 쓰는 트리플의 숫자를 저장하는 변수.
    if (CP == 4)//플러시 보다 우선 순위
    {
