...
{
    sellingItems.push_back(Item("나무 카드", 10, 100));
    sellingItems.push_back(Item("대나무 카드", 20, 500));
    sellingItems.push_back(Item("돌멩이 카드", 30, 1000));
    sellingItems.push_back(Item("장난감 카드", 100, 5000));
    sellingItems.push_back(Item("청동 카드", 200, 10000));
    sellingItems.push_back(Item("강철 카드", 1000, 100000));
    sellingItems.push_back(Item("다이아 카드", 10000, 1000000));
}
...
void StoreScene::storeInformation(Player* player, InventoryScene* inventory)
{
    CursorControl cursor;
    cursor.gotoxy(6, 23);
    std::cout << "현재 골드 " << player->GetMoney();
